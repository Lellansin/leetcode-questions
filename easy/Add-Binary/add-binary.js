...
var strBinAdd = function(a, chs) {
	...
	for (i = chs.length - 1; len > 0; i--) {
		addBit(arr, parseInt(chs[i]) || 0, len--);
	}
...
var addBit = function(num, bin, bit) {
	...
	switch (tmp) {
		...
		default:
			throw new Error('num[bit] is ' + num[bit] + ', bit is ' + bit + ', bin is ' + bin + ', tmp is ' + tmp);
	}
}
